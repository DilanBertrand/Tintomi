    ...
    <Card title="Idea box" subtitle="Shape what we build next">
      <AnimatePresence mode="wait" initial={false}>
        {status === 'sent' ? (
          ...
          >
            <p className="max-w-sm text-sm font-medium text-[#e9ece8]">
              Thank you! Your idea has been sent to the Founder.
            </p>
            <button
              type="button"
              onClick={() => setStatus('idle')}
              className="min-h-12 shrink-0 rounded-lg border border-[#232b25] bg-transparent px-4 py-3 text-sm font-medium text-[#e9ece8] transition-colors hover:bg-[#1a221c]"
            >
              Send another
            </button>
          </motion.div>
          ...
          >
            <p className="max-w-sm text-sm text-[#a7b0a8]">
              Got a lesson or project you wish existed? Tell the founder directly.
            </p>
            <button
              type="button"
              onClick={() => setStatus('open')}
              className="min-h-12 shrink-0 rounded-full bg-[#e9ece8] px-6 py-3 text-sm font-semibold text-[#0f1412] transition-opacity duration-200 hover:opacity-90"
            >
              Give us ideas for new lessons/projects
            </button>
          </motion.div>
        ) : (
          <motion.div
